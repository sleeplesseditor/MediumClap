...
const useClapState = (initialState = INITIAL_STATE, reducer = internalReducer) => {
  ...
  return {clapState, updateClapState, getClickProps, getCountProps, reset, resetDependency: resetRef.current}
}
...
const ClapCount = ({ count, setRef, ...restProps }) => {
  return <span className="count"  ref={setRef} {...restProps}>
    + {count}
  </span>
}

const CountTotal = ({ countTotal, setRef, ...restProps }) => {
  return <span className="total" ref={setRef} {...restProps}>
    {countTotal}
  </span>
}
...
const Usage = () => {
  ...
  const reducer = (state, action) => {
    if(action.type === useClapState.typees.clap && isClappedTooMuch) {
      return state
    }
  ...
  return (
    <div className="clap-container-card-content">
      ...
      <section className="reset-container">
        ...
        <pre className="reset-msg">
          {JSON.stringify({timesClapped, count, countTotal})}
        </pre>
        <pre className="reset-msg">
          {uploadingReset ? `Uploading Reset ${resetDependency} ...` : ''}
        </pre>
        <pre className="reset-msg-danger">
