...
import styles from './MediumClap.scss';
import userCustomStyles from './usage.scss';
...
const CompoundComponentClap = ({ children, onClap, style : userStyles = {}, className }) => {
  ...
  const providerClassName = ['clap', className].join(' ').trim()
...
const ClapIcon = ({ style: userStyles = {}, className }) => {
  ...
  const iconClass = ['icon', isClicked ? 'checked' : '', className]
    .join(' ')
    .trim()
...
const ClapCount = ({ style: userStyles = {}, className }) => {
  ...
  const clapCountClass = ['count', className].join(' ').trim();
  return (
    <span 
      className={clapCountClass}
...
const CountTotal = ({ style: userStyles = {}, className }) => {
  ...
  const countTotalClass = ['total', className].join(' ').trim();
  return (
    <span 
      className={countTotalClass} 
...
const Usage = () => {
  ...
  return (
    <div style={{ width: '100%' }}>
      <CompoundComponentClap onClap={handleClap} className={'darkClap'} >
        <CompoundComponentClap.Icon className={'darkIcon'} />
        <CompoundComponentClap.Count className={'darkCount'} />
        <CompoundComponentClap.Total className={'darkTotal'} />
